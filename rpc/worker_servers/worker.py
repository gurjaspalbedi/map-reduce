...
from worker_servers.messages import welcome
...
from worker_servers.configuration import (
    worker_list,
    inverted_index_path,
    word_count_path,
    word_count_map,
...
from data_store.rpc_constants import (
    INITIAL_STAGE,
    INTERMEDIATE_STAGE,
    FINAL_STAGE,
    TASK_INVERTED_INDEX,
...
def destroy_cluster(cluster_id):
    ...
    if len(clusters[cluster_id]) == 0:
        log.write(
            "Either there is not cluster initialized or cluster_id is wrong", "error"
        )
    else:
        ...
        for file in files:
            with open(file, "r") as f:
                ...
                while not_done:
                    ...
                    if len(line) > 0:
                        with open(
                            f"{mapper_tasks_path}task{task_count}", "wb"
                        ) as task_file:
        ...
        while True:
            ...
            if command == "1":
                ...
                try:
                    ...
                    log.write(
                        'init_cluster([("127.0.0.1", 50053), ("127.0.0.1", 50054 )])',
                        "debug",
                    )
                ...
                try:
                    ...
                    log.write(
                        "run_mapred(0, 0) <first_argument: cluster_id> <second_argument: task_id>",
                        "error",
                    )
                    log.write(
                        "task should be 0 for WORD_COUNT  and 1 for Inverted Index",
                        "error",
                    )
            else:
